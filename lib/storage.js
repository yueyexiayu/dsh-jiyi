...
async function ensureScope(scopeDir) {
  ...
  await fs.mkdir(nodePath.join(scopeDir, "observations", "_conflicts"), { recursive: true });
...
export async function clearArchive(scopeDir) {
  const files = await listMd(nodePath.join(scopeDir, "archive"), "archive", scopeDir);
