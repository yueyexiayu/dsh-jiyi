...
import { clearArchive, listMd, listWorkspaceScopeDirs, regenerateManifest } from "./storage.js";
...
async function discardInbox(inbox, scopeDir) {
  ...
  if (scopeDir) await clearArchive(scopeDir);
}

async function moveToConflicts(files, scopeDir) {
  const dir = nodePath.join(scopeDir, "observations", "_conflicts");
  await fs.mkdir(dir, { recursive: true });
  for (const file of files) {
    try {
      await fs.rename(file.path, nodePath.join(dir, file.name));
    } catch (error) {
      if (!error || error.code !== "ENOENT") throw error;
    }
  }
...
export async function dreamScope(scopeDir, options = {}) {
  ...
  const topicsDir = nodePath.join(scopeDir, "topics");
  const loaded = await loadMarkdownDir(inboxDir, "observations/_inbox", scopeDir);
  ...
  if (conflicted.length) await moveToConflicts(conflicted, scopeDir);
  if (inbox.length === 0) {
    await clearArchive(scopeDir);
    await regenerateManifest(scopeDir);
    return { merged: 0, topics: [], remaining: 0, via: "noop", conflicts: conflicted.length };
  }
  ...
  if (Array.isArray(options.routes)) {
    ...
    if (dreamPlanIsEmpty(plan)) {
      ...
      await discardInbox(still, scopeDir);
      await regenerateManifest(scopeDir);
  ...
  if (dreamPlanIsEmpty(plan)) {
    await discardInbox(live, scopeDir);
    await regenerateManifest(scopeDir);
