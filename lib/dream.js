...
export async function dreamScope(scopeDir, options = {}) {
  ...
  const currentStamp = await scopeStamp(scopeDir);
  if (currentStamp !== stamp) {
    if (!options.retried) {
      return dreamScope(scopeDir, { ...options, retried: true, ignoreSlugs: [...ignoreSlugs] });
    }
    const liveAfter = await liveInbox(inbox);
    ...
    if (dreamPlanIsEmpty(plan)) {
      await archiveInbox(liveAfter, archiveDir, scopeDir);
      await regenerateManifest(scopeDir);
      return { merged: liveAfter.length, topics: [], remaining: 0, via: "noop" };
    }
  }
