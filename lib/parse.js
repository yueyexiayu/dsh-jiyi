...
export function truncateUtf8(text, maxBytes, suffix = "\n…\n") {
  ...
  let budget = Math.max(0, maxBytes - suffixBuf.length);
  while (budget > 0 && (encoded[budget] & 0xc0) === 0x80) budget -= 1;
  return Buffer.concat([encoded.subarray(0, budget), suffixBuf]).toString("utf8");
}
